...
# 🌟 1. แวะไปดูใน Firebase ก่อนว่าเคยตั้งเวลาอะไรไว้ไหม
alert_time_ref = db.reference('system_status/alert_time')
saved_time_str = alert_time_ref.get()

# 🌟 2. แปลงข้อมูลเวลาจาก Firebase ให้ Streamlit เข้าใจ
if saved_time_str:
    h, m = map(int, saved_time_str.split(':'))
    default_alert_time = dt_time(h, m)
else:
    default_alert_time = dt_time(8, 0) # ถ้าเพิ่งเปิดครั้งแรก ให้เริ่มที่ 08:00

# 🌟 3. สร้างช่องให้ผู้ใช้เลือกเวลา (โดยใช้ค่าจาก Firebase เป็นค่าเริ่มต้น)
alert_time = st.sidebar.time_input("เลือกเวลาส่งสรุปผล", value=default_alert_time)

# 🌟 4. ตรวจสอบว่า "ผู้ใช้เพิ่งกดเปลี่ยนเวลาใหม่ใช่ไหม?"
current_time_str = alert_time.strftime('%H:%M')
if current_time_str != saved_time_str:
    # ถ้าเปลี่ยนเวลาปุ๊บ ให้เซฟทับลง Firebase ทันที!
    alert_time_ref.set(current_time_str)
...
if page == "🏠 หน้าหลัก (Dashboard)":
    ...
    if not df_night.empty:
        ...
        # ---------------------------------
        # 🌟 ระบบเช็คเวลาส่ง LINE อัตโนมัติ (ใช้ Firebase จำค่าแทน Session State)
        # ---------------------------------
        ...
        if thai_time >= target_alert_datetime:
            # แวะไปดูสมุดจดใน Firebase ว่าวันนี้ส่งหรือยัง
            last_sent_ref = db.reference('system_status/last_sent_date')
            last_sent_db = last_sent_ref.get()
            
            # ถ้าในฐานข้อมูลยังไม่ใช่วันที่ของวันนี้ แปลว่ายังไม่ได้ส่ง!
            if last_sent_db != str(current_date):
                status = send_line_message(report_msg)
                
                if status == 200:
                    # ส่งเสร็จปุ๊บ จดลง Firebase ทันทีว่าวันนี้ส่งแล้ว
