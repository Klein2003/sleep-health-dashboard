...
# ==========================================
# 4. ดึงข้อมูล และ ระบบ Auto-Cleanup
# ==========================================
...
if all_sleep_data_nested:
    ...
    # 🌟 แกะกล่องข้อมูล (แก้บั๊กแยกระหว่าง ข้อมูลแบบเก่า vs แบบใหม่)
    for key, value in all_sleep_data_nested.items():
        if isinstance(value, dict):
            # ตรวจสอบว่ามีคีย์ 'time' อยู่ชั้นนอกสุดไหม (ถ้ามีแปลว่าเป็นข้อมูลโครงสร้างเก่า)
            if 'time' in value:
                flat_sleep_data[key] = value
                fake_date = key.split(" ")[0]
                if fake_date not in avail_dates_list:
                    avail_dates_list.append(fake_date)
            else:
                # โครงสร้างใหม่แบบมีโฟลเดอร์วันที่
                if key < cutoff_date_str:
                    folders_to_delete.append(key)
                else:
                    flat_sleep_data.update(value)
                    if key not in avail_dates_list:
                        avail_dates_list.append(key)

    # ลบโฟลเดอร์เก่าทิ้งยกเข่ง!
...
# 🌟 ระบบบอกใบ้ว่าวันไหนมีข้อมูลบ้าง
if avail_dates_list:
    try:
        ...
        # เผื่อวันที่โครงสร้างเก่ามี format แปลกๆ
        st.sidebar.success(f"✅ มีข้อมูลบันทึกในระบบ")
else:
